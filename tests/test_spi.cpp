...
// Tests (simulated) writing a value to the SPI bus by calling SpiEmulator::write() for a single value and then checking
// the internal buffer to verify the value was written correctly
...
// Tests (simulated) receiving a value from the SPI bus by forcing a single value into the receive buffer via a pointer
// and then verifying the same value is received by a call to SpiEmulator::read()
...
// Simulates writing 255 to the bus, then writing all the numbers from 0 to 255. The output is "looped back" to the
// input, so that value of each read() should be the same value written by test_loopback() on the previous cycle
